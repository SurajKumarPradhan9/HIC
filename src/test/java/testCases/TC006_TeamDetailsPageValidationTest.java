...
public class TC006_TeamDetailsPageValidationTest extends BaseTest {
    ...
    public void isHeadingPresent() {
        ...
        try {
            log.info("STEP ➜ Validate Team Details heading is present/visible");
    ...
    public void isTeamDetailsPresent() {
        ...
        try {
            log.info("STEP ➜ Validate Team member details section is present/visible");
            boolean isTeamDetailsPresent = td.isTeamDetailsPresent();
            log.debug("Team details present: {}", isTeamDetailsPresent);

            Assert.assertTrue(isTeamDetailsPresent, "Team details not present");
            log.info("PASS ✓ STEP: Team details present");
        } catch (AssertionError ae) {
            log.error("ASSERTION FAILED ✗ (Team details present): {}", ae.getMessage(), ae);
            throw ae;
        } catch (Exception e) {
            log.error("ERROR ✗ (Team details present) threw unexpected exception: {}", e.getMessage(), e);
            Assert.fail("Unexpected error while checking Team details presence: " + e.getMessage(), e);
        }
    }
